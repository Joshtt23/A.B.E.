    ...
    with ThreadPoolExecutor(max_workers=Config.MAX_WORKERS) as executor:
        ...
        with ProcessPoolExecutor() as cpu_executor:
            ...
            for i, article in enumerate(articles):
                ...
                logging.info(f"Submitted Sentiment Transformer task for article {i + 1}/{len(articles)}")
                ...
                summary_transformer_task = executor.submit(processor.summary_generator.generate_summary, text)
                logging.info(f"Submitted Summary Transformer task for article {i + 1}/{len(articles)}")
                ...
                logging.info(f"Submitted Keywords YAKE task for article {i + 1}/{len(articles)}")
            ...
            for timestamp, analyzed_data, sentiment_transformer_task, sentiment_spacy_task, summary_spacy_task, summary_transformer_task, keywords_rake_task, keywords_textrank_task, keywords_spacy_task, keywords_huggingface_task, keywords_yake_task in article_tasks:
                ...
                # logging.info(f"Sentiment Transformer task completed for article {i + 1}/{len(articles)}: {sentiment_transformer_task.done()}")
                ...
                # logging.info(f"Summary Transformer task completed for article {i + 1}/{len(articles)}: {summary_transformer_task.done()}")
                ...
                logging.info(f"Keyword Yake task completed for article {i + 1}/{len(articles)}: {keywords_yake_task.done()}")
